...
describe('imageValidator.ts', () => {
  ...
  beforeEach(() => {
    image = new ImageForm(
      1,
      '',
      new File(['Hello World.'], 'testFile.txt', { type: 'text/plain' }),
      new Image(1200, 900)
    )
  })
  ...
  it('should return errors when image has the wrong mime type', () => {
    ...
    expect(errors).toStrictEqual([
      new Error(
        'Die Datei beinhaltet einen falschen Dateityp (text/plain). Zugelassen sind nur die folgenden Dateitypen image/jpeg, image/webp.'
      )
    ])
  })
  it('should return errors when image has the wrong sizeInBytes', () => {
    ...
    expect(errors).toStrictEqual([
      new Error(
        'Die maximale Dateigröße beträgt 0,00\u00A0B. Die Dateigröße des Bildes beträgt 12,00\u00A0B.'
      )
    ])
  })
  it('should return errors when image has the wrong width', () => {
    ...
    expect(errors).toStrictEqual([
      new Error(
        'Die Breite des eingegebenen Bildes misst 1200 px. Die akzeptierte Breite ist 500 px.'
      )
    ])
  })
  it('should return errors when image has the wrong height', () => {
    ...
    expect(errors).toStrictEqual([
      new Error('Die Höhe des eingegebenen Bildes misst 900 px. Die akzeptierte Höhe ist 600 px.')
    ])
  })
  it('should return multiple errors when image has multiple conflicts', () => {
    ...
    expect(errors).toStrictEqual([
      new Error(
        'Die Datei beinhaltet einen falschen Dateityp (text/plain). Zugelassen sind nur die folgenden Dateitypen image/png.'
      ),
      new Error(
        'Die maximale Dateigröße beträgt 1,00\u00A0B. Die Dateigröße des Bildes beträgt 12,00\u00A0B.'
      ),
      new Error(
        'Die Breite des eingegebenen Bildes misst 1200 px. Die akzeptierte Breite ist 100 px.'
      ),
